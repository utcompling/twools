...
def getTweets(userID):
    ...
    print "\tRetrieving Timeline for User: " + str(userID)

    if userID in processedOthers:
    ...
    while attempts < maxAttempts:
        try:
            attempts += 1
            tweets = api.GetUserTimeline(id = userID, count = numTweetsPerTimeline)
            ...
            print ("\t\tAttempt %d: Failed retrieval for User: " + str(userID)) % attempts
...
#===============================================================================
# Grab user's tweet and outputs it to file
#===============================================================================
def outputUserTweets(userID):
    tweets = getTweets(userID)
    idString = ("USER_" + str(userID) + "\t")
    for tweet in tweets:
...
def main():
    ...
    setup.add_argument('-u',
                        ...
                        metavar = 'FILE',
                        required = True)
    graphs.add_argument('-t',
                        action = "store",
                        metavar = 'Tweets Output',
                        help = 'Grab tweets of all users from the input file.')
    ...
    #Create output files
    if args.t:
        global outUserTweets
        outUserTweets = codecs.open(args.t, encoding = 'utf-8', mode = 'w')
